...
async function generatePDF(qaItems, formTitle) {
  return new Promise((resolve) => {
    ...
    // Add title
    doc.fontSize(24).text(formTitle, { align: "center" }).moveDown(2);
    ...
    qaItems.forEach((item, index) => {
      ...
      // Answers - using letters instead of numbers
      item.answers.forEach((answer, ansIndex) => {
        const letterLabel = String.fromCharCode(65 + ansIndex); // 65 is ASCII for 'A'
        doc.fontSize(12);
        if (answer.isCorrect) {
          doc.font("Times-Bold");
        } else {
        ...
        doc.text(`${letterLabel}. ${answer.text}`).moveDown(0.5);
      });
...
export async function POST(req) {
  try {
    ...
    // Get the form title
    const formTitle =
      $(".F9yp7e").text().trim() || "Form Questions and Answers";
    ...
    $(".OxAavc").each((index, element) => {
      ...
      $(element)
        ...
        .each((i, answerElem) => {
          ...
          if (!uniqueAnswers.has(answerText)) {
            const isCorrect =
              $(answerElem).closest(".yUJIWb").find(".fKfAyc").text().trim() ===
              "Tama";
            answers.push({
              text: answerText,
              isCorrect: isCorrect,
